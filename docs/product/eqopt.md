...
## A founder puts a floor under a concentrated holding
...
- **Strike** — The price at which the option lets its holder buy or sell the underlying.
- **Premium** — The amount paid upfront for the option, and the most a buyer can lose on it.
- **Implied volatility** — The volatility figure that makes a model return the option’s traded price — the language dealers quote in.
- **Delta** — How much the option’s value moves for a small move in the underlying, and so how much stock hedges it.
- **Vega** — How much the option’s value moves when implied volatility changes by one point.
- **Barrier** — A price level that brings an option into existence or extinguishes it if the underlying trades through it.
