...
## A five-year note that defaults in year four
...
- **Special purpose vehicle** — A standalone issuing entity created for one deal, holding the collateral and the swap that make the note work.
- **Embedded CDS** — The credit default swap inside the note, on which the investor is the protection seller.
- **Collateral** — The high-quality assets the note proceeds are invested in and pledged against, in a vehicle-issued structure.
- **Enhanced coupon** — The above-market interest the note pays, made up of the collateral return plus the protection premium.
- **Contingent principal** — Repayment that is due in full only if no credit event occurs, and falls to recovery value if one does.
- **Funded protection** — Protection paid for in cash at the outset, so the buyer has nothing left to collect from the seller later.
