...
[Open in the app](/OTC_Learn/) · [FX](/OTC_Learn/category/fx/) · foundational
...
## A US importer fixes a euro payable
...
- **Spot rate** — The rate for an exchange settling on the standard spot date, normally two business days after the trade.
- **Forward points** — The adjustment added to or subtracted from spot to give the forward rate, driven by the interest rate differential.
- **Value date** — The date on which the two currencies are actually delivered to each other.
- **Base currency** — The first currency in a pair — the one whose price is being quoted, so EUR/USD is dollars per euro.
- **Outright forward** — A single exchange of two currencies on one future date, with no offsetting near leg.
- **Window forward** — A forward the client may settle on any date within an agreed range rather than one fixed date.
...
- [Non-Deliverable Forward](/OTC_Learn/product/ndf/) — A cash-settled forward for restricted currencies
